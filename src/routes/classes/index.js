...
function checkUser(req, res, next) {
  ...
  const userType = req.user["account_type"];
  if (userType === "teacher") {
    next();
  } else {
  ...
  Classes.findClass(id)
    .then((classes) => {
      if (classes) {
        ...
        res
          .status(401)
          .json({ message: "Cannot find a class with the given id" });
      }
    })
    .catch((err) => {
      res.status(500).json({ message: "cannot find classes" });
...
classHandler.post("/", checkUser, (req, res) => {
  ...
  if (newClass) {
    Classes.addClass({ ...newClass, instructor_id: req.user.id })
      .then((createdClass) => {
      ...
      .catch((err) => {
        ...
        res
          .status(401)
          .json({ message: "Please provide required credentials" });
      });
  } else {
...
classHandler.put("/:id", checkUser, (req, res) => {
  ...
  Classes.findClass(id)
    .then((classes) => {
      if (classes) {
        Classes.editClass(edits, id).then((editedClass) => {
  ...
  Classes.deleteClass(id)
    .then((remove) => {
      if (remove) {
        ...
        res
          .status(401)
          .json({ message: "could not find that class to delete" });
      }
    })
    .catch((err) => {
